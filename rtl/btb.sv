...
(
    ...
    // Internal nets.
    //---------------------------------
    logic [BIA_WIDTH   - 1:0] bia_read;  // Branch instruction address.
    logic [INDEX_WIDTH - 1:0] index_read;

    logic                    hit;
    logic [N          - 1:0] hit_find;
    logic [$clog2 (N) - 1:0] way_read;
    ...
    //-----------------------------------
    assign bia_read   = pc_i[BIA_MSB  :BIA_LSB  ];
    assign index_read = pc_i[INDEX_MSB:INDEX_LSB];

    assign btb_update = branch_taken_i & (~ stall_if_i);
    ...
    always_comb begin
        hit_find[0] = valid_mem[index_read][0] & (bia_mem[index_read][0] == bia_read);
        hit_find[1] = valid_mem[index_read][1] & (bia_mem[index_read][1] == bia_read);
        hit_find[2] = valid_mem[index_read][2] & (bia_mem[index_read][2] == bia_read);
        hit_find[3] = valid_mem[index_read][3] & (bia_mem[index_read][3] == bia_read);

        casez ( hit_find )
            4'bzzz1: way_read = 2'b00;
            4'bzz10: way_read = 2'b01;
            4'bz100: way_read = 2'b10;
            4'b1000: way_read = 2'b11;
            default: way_read = plru; // If there is no record of this branch instruction, new_value will be written into place of plru.
        endcase
    end
    ...
    assign plru = {plru_mem[index_read][0], (plru_mem[index_read][0] ? plru_mem[index_read][2] : plru_mem[index_read][1])};
    ...
    always_ff @(posedge clk_i, posedge arst_i) begin
        ...
        end else if (btb_update) valid_mem[index_write_i][way_write_i] <= 1'b1;
    end
    ...
    //------------------------------
    assign hit_o         = hit;
    assign target_addr_o = bta_mem[index_read][way_read];

    assign way_write_o = way_read;
