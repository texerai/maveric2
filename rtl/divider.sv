...
// ------------------------------------------------------------------
// This is Multi-cycle RISC-V M-extension compliant divider.
...
(
    ...
    // Data inputs.
    input  logic [DATA_WIDTH - 1:0] a_i,             // Dividend.
    input  logic [DATA_WIDTH - 1:0] b_i,             // Divisor.

    // Output interface.
    output logic [DATA_WIDTH - 1:0] c_o,             // Quotient or Remainder.
    output logic                    done_o
);

    localparam MAX_CYCLES = DATA_WIDTH + 1;
    ...
    logic [2*DATA_WIDTH - 1:0]        remainder_s;
    logic [DATA_WIDTH - 1:0]          quotient_s;
    logic [$clog2(DATA_WIDTH) - 1:0]  cycle_counter_s;
    ...
    logic [DATA_WIDTH - 1:0]     special_result_s;

    // Combinational: sign and unsigned magnitude of operands.
    logic                        start_dividend_negative_s;
    logic                        start_divisor_negative_s;
    logic [DATA_WIDTH - 1:0]     start_dividend_unsigned_s;
    logic [DATA_WIDTH - 1:0]     divisor_unsigned_s;

    // Is dividend A negative (signed op only)?
    assign start_dividend_negative_s = (op_i == 2'b00 || op_i == 2'b10)
                                       ? (is_mdu_word_op_i ? a_i[DATA_WIDTH/2 - 1] : a_i[DATA_WIDTH - 1])
                                       : 1'b0;

    // Is divisor B negative (signed op only)?
    assign start_divisor_negative_s  = (op_i == 2'b00 || op_i == 2'b10)
                                       ? (is_mdu_word_op_i ? b_i[DATA_WIDTH/2 - 1] : b_i[DATA_WIDTH - 1])
                                       : 1'b0;

    assign start_dividend_unsigned_s = start_dividend_negative_s
                                       ? (is_mdu_word_op_i ? {{(DATA_WIDTH/2){1'b0}}, (~a_i[DATA_WIDTH/2 - 1:0] + 1)}
                                                           :                     (~a_i + 1))
                                       : (is_mdu_word_op_i ? {{(DATA_WIDTH/2){1'b0}},   a_i[DATA_WIDTH/2 - 1:0]}
                                                           :                        a_i);

    // Divisor unsigned: derived from stored sign — stable during computation.
    assign divisor_unsigned_s        = divisor_negative_s
                                       ? (is_mdu_word_op_i ? {{(DATA_WIDTH/2){1'b0}}, (~b_i[DATA_WIDTH/2 - 1:0] + 1)}
                                                           :                     (~b_i + 1))
                                       : (is_mdu_word_op_i ? {{(DATA_WIDTH/2){1'b0}},   b_i[DATA_WIDTH/2 - 1:0]}
                                                           :                        b_i);

    // Iteration combinational logic.
    // temp is 129-bit: bit [2*DATA_WIDTH] is the true borrow flag, correct even when
    // divisor_unsigned_s >= 2^(DATA_WIDTH-1) (where a 128-bit MSB check would fail).
    logic [2*DATA_WIDTH - 1:0] remainder_shifted_s;
    logic [2*DATA_WIDTH:0]      temp_s;

    assign remainder_shifted_s = remainder_s << 1;
    assign temp_s              = {1'b0, remainder_shifted_s} - {1'b0, divisor_unsigned_s, {DATA_WIDTH{1'b0}}};
    ...
    always_ff @(posedge clk_i, posedge arst_i) begin
        ...
        if (arst_i) begin
            ...
            if (is_mdu_word_op_i ? (b_i[DATA_WIDTH/2 - 1:0] == '0) : (b_i == '0)) begin
                ...
                case (op_i)
                    2'b00:   special_result_s <= {DATA_WIDTH{1'b1}};
                    2'b01:   special_result_s <= {DATA_WIDTH{1'b1}};
                ...
                case (op_i)
                    2'b00:   special_result_s <= {{(DATA_WIDTH - 1){1'b0}}, 1'b1};
                    2'b01:   special_result_s <= {{(DATA_WIDTH - 1){1'b0}}, 1'b1};
                ...
                is_mdu_word_op_i
                ? (a_i[DATA_WIDTH/2 - 1:0] == {1'b1, {(DATA_WIDTH/2 - 1){1'b0}}} && b_i[DATA_WIDTH/2 - 1:0] == {(DATA_WIDTH/2){1'b1}})
                : (a_i               == {1'b1, {(DATA_WIDTH   - 1){1'b0}}} && b_i               == {DATA_WIDTH{1'b1}})
            )) begin
                ...
                special_case_s       <= 1'b0;
                remainder_s          <= {{DATA_WIDTH{1'b0}}, start_dividend_unsigned_s};
                quotient_s           <= '0;
                cycle_counter_s      <= '0;
            ...
            if (temp_s[2*DATA_WIDTH]) begin
                ...
                remainder_s <= temp_s[2*DATA_WIDTH - 1:0];
                quotient_s  <= {quotient_s[DATA_WIDTH - 2:0], 1'b1};

            end

            cycle_counter_s <= cycle_counter_s + 1;

            if (cycle_counter_s == ($clog2(DATA_WIDTH))'(MAX_CYCLES - 2))
                busy_s <= 1'b0;
    ...
    // Result computation.
    //-------------------------------------
    logic [DATA_WIDTH - 1:0] quotient_result_s;
    logic [DATA_WIDTH - 1:0] remainder_result_s;

    always_comb begin

        remainder_result_s = remainder_s[2*DATA_WIDTH - 1:DATA_WIDTH];
    ...
    always_comb begin
        ...
        if (special_case_s) begin
            ...
            c_o = is_mdu_word_op_i
                ? {{(DATA_WIDTH/2){special_result_s[DATA_WIDTH/2 - 1]}}, special_result_s[DATA_WIDTH/2 - 1:0]}
                : special_result_s;
            ...
            case (op_stored_s)
                ...
                2'b00, 2'b01: begin
                    c_o = is_mdu_word_op_i
                        ? {{(DATA_WIDTH/2){quotient_result_s[DATA_WIDTH/2 - 1]}},  quotient_result_s[DATA_WIDTH/2 - 1:0]}
                        : quotient_result_s;
                end

                2'b10, 2'b11: begin
                    c_o = is_mdu_word_op_i
                        ? {{(DATA_WIDTH/2){remainder_result_s[DATA_WIDTH/2 - 1]}}, remainder_result_s[DATA_WIDTH/2 - 1:0]}
                        : remainder_result_s;
                end
