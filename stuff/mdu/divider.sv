...
module divider #(
    ...
    // which one to keep based on temp[MSB].

    assign remainder_shifted = remainder << 1;
    assign temp = remainder_shifted - {divisor_unsigned, {XLEN{1'b0}}};
    ...
    always_ff @(posedge clk) begin
        ...
        if (rst) begin
            ...
            else if ((op == 2'b00) && (A == {1'b1, {(XLEN-1){1'b0}}}) && (B == {XLEN{1'b1}})) begin // signed overflow
            ...
            if (temp[2*XLEN-1]) begin // if temp[MSB] == 0

                // result >= 0 -> divisor fits -> remainder <= temp -> quotient bit = 1
			...
			else begin // if temp[MSP] == 1

                // result < 0 -> divisor doesn't fit -> remainder <= remainder_shifted (restore), quotient bit = 0
